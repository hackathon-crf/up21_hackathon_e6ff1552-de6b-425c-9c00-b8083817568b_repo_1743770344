...
export default function Home() {
	...
	useEffect(() => {
		...
		const forceRedirectTimer = setTimeout(() => {
			console.log("🔍 ROOT PAGE: FORCE REDIRECT - Auth state took too long, forcing redirect");
	...
	useEffect(() => {
		...
		if (forceRedirect) {
			console.log(`🔍 [${debugId}] ROOT PAGE: Force redirect parameter detected, bypassing auth checks`);
		...
		if (isLoading) {
			console.log(`🔍 [${debugId}] ROOT PAGE: Auth still loading, not redirecting yet`);
			return;
		}
		...
		if (redirectTimestamp && now - parseInt(redirectTimestamp) < 5000) {
			console.log(`🔍 [${debugId}] ROOT PAGE: Recent redirect detected, preventing loop`, {
				lastRedirect: new Date(parseInt(redirectTimestamp)).toISOString(),
				timeSince: now - parseInt(redirectTimestamp) + 'ms'
			});
			return;
		}
