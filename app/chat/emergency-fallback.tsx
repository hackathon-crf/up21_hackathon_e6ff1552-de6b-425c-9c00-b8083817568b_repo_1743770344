...
export function EmergencyFallback() {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      <h1 style={{ color: '#333', marginBottom: '20px' }}>Emergency Fallback UI</h1>
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {sessions.map(session => (
            <li key={session.id} style={{
              padding: '10px 15px',
              borderLeft: session.is_pinned ? '4px solid #4a90e2' : '1px solid #ddd',
              marginBottom: '10px',
      ...
      <p>If you're seeing this, it means there was an issue with the main chat components.</p>
...
export function injectEmergencyFallback() {
  try {
    ...
    if (React && React.createElement && React.render) {
