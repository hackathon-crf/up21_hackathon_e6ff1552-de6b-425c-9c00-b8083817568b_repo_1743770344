...
function logAuthEvent(level: 'info' | 'warn' | 'error' | 'critical', message: string, data?: any) {
  const timestamp = new Date().toISOString();
  const prefix = level === 'critical' ? '🚨 AUTH CRITICAL:' : 
                level === 'error' ? '❌ AUTH ERROR:' :
                level === 'warn' ? '⚠️ AUTH WARNING:' : '📝 AUTH INFO:';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!session) {
      logAuthEvent('warn', "No active session", { cookieCount: Object.keys(cookieStore.getAll()).length });
      return NextResponse.json({
        authenticated: false,
