...
export function GameChat({ sessionId, className, collapsed = false, onToggleCollapse }: GameChatProps) {
    ...
    const initialMessages: Message[] = [
      {
        ...
        content: "Welcome to the CPR & AED Challenge session! Get ready to test your knowledge.",
      ...
      },
      {
        ...
        content: "Hi everyone! Let's wait for all players to be ready before we start.",
      ...
      if (Math.random() > 0.7) {
        ...
        setTimeout(() => {
          ...
          setMessages((prev) => {
            ...
            const filtered = prev.filter((msg) => msg.id !== `typing-${Date.now()}`)
  ...
  if (collapsed) {
    return (
      <Button
        variant="outline"
        className={cn("fixed bottom-4 right-4 z-50 flex items-center gap-2", className)}
  ...
  return (
    <Card className={cn("flex h-[400px] flex-col pb-0 pt-0 gap-0", className)}>
            ...
            {messages.map((message) => {
              if (message.type === "event") {
                if (message.content === "typing") {
                  return (
                    <div key={message.id} className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              return (
                <div key={message.id} className={cn("flex gap-2", isCurrentUser ? "flex-row-reverse" : "flex-row")}>
                  ...
                  >
                    {!isCurrentUser && <p className="mb-1 text-xs font-medium">{message.sender.name}</p>}
                    <p>{message.content}</p>
                    <p
                      className={cn(
