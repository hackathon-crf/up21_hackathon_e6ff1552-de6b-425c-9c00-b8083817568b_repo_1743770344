...
const PlayerListComponent = ({
  ...
  const instanceId = useRef(`list-${Math.random().toString(36).substr(2, 9)}`).current
  ...
  console.log(`%c[PlayerList ${instanceId}] Component instance`, debugColors.playerList)
  
  // Debug players data whenever it changes
  useEffect(() => {
    console.log(`%c[PlayerList ${instanceId}] Players data changed:`, debugColors.playerList, players)
  }, [players, instanceId])

  const [expandedPlayer, setExpandedPlayer] = useState<string | number | null>(null)
  
  // Debug state changes
  useDebugState(expandedPlayer, 'expandedPlayer', `PlayerList ${instanceId}`, debugColors.playerList)
  ...
  useEffect(() => {
    console.log(`%c[PlayerList ${instanceId}] Ready status: ${readyCount}/${totalPlayers} (allReady: ${allReady})`, debugColors.playerList)
  }, [readyCount, totalPlayers, allReady, instanceId])

  // Use useCallback to memoize the click handler
  const handlePlayerClick = useCallback((playerId: string | number) => {
    console.log(`%c[PlayerList ${instanceId}] handlePlayerClick called with id: ${playerId}`, debugColors.playerList)
    setExpandedPlayer(prevId => {
      const newValue = prevId === playerId ? null : playerId
      console.log(`%c[PlayerList ${instanceId}] setExpandedPlayer: ${prevId} -> ${newValue}`, debugColors.playerList)
      return newValue
    })
  }, [instanceId])

  // Debug before creating player items
  console.log(`%c[PlayerList ${instanceId}] Before creating player items, render #${renderCount}`, debugColors.playerList)
  ...
  const playerItems = players.map((player) => {
    console.log(`%c[PlayerList ${instanceId}] Creating item for player:`, debugColors.playerList, player)
    ...
    return (
      <div
        ...
        className={cn(
          ...
          player.isCurrentUser ? "bg-primary/10" : "bg-muted/50 hover:bg-muted/80",
      ...
      >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {player.isHost && (
                <Badge
                  variant="outline"
                  className="border-yellow-500 text-yellow-500 flex items-center gap-1 text-xs"
                >
        ...
        <div className="flex items-center gap-2">
          ...
          {showControls && isCurrentUserHost && !player.isCurrentUser && expandedPlayer === player.id && (
            <div className="flex items-center gap-1">
              {onPromotePlayer && (
                <Button
                  ...
                  onClick={() => {
                    console.log(`%c[PlayerList ${instanceId}] Promote button clicked for player: ${player.id}`, debugColors.playerList)
              ...
              {onKickPlayer && (
                <Button
                  ...
                  className="h-7 px-2 text-xs"
                  onClick={() => {
                    console.log(`%c[PlayerList ${instanceId}] Kick button clicked for player: ${player.id}`, debugColors.playerList)
  ...
  useEffect(() => {
    if (playerItemsRef.current.length !== playerItems.length) {
      console.log(`%c[PlayerList ${instanceId}] Player items count changed: ${playerItemsRef.current.length} -> ${playerItems.length}`, debugColors.playerList)
      playerItemsRef.current = playerItems
    } else {
      console.log(`%c[PlayerList ${instanceId}] Player items recreated with same count: ${playerItems.length}`, debugColors.playerList)
    }
  }, [playerItems, instanceId])

  console.log(`%c[PlayerList ${instanceId}] Before final render, render #${renderCount}`, debugColors.playerList)

  // Track when the component is mounted/unmounted
  useDebugEffect('lifecycle', `PlayerList ${instanceId}`, debugColors.playerList, [])

  return (
    <Card className={cn("", className)}>
      <CardHeader className="p-4 pb-2 pt-0 pb-0">
      ...
      <CardContent className="p-4 pt-2">
        ...
        {onToggleReady && (
          <Button 
            ...
            onClick={() => {
              console.log(`%c[PlayerList ${instanceId}] Toggle ready button clicked`, debugColors.playerList)
              onToggleReady()
            }}
          >
