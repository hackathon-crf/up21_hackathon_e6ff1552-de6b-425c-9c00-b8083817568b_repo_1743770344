...
const ConnectionStatusComponent = ({
	...
	const renderCount = useDebugRenders(
		"ConnectionStatus",
		debugColors.connectionStatus,
		{
			className,
			showLabel,
			showTooltip,
	...
	const instanceId = useRef(
		`conn-${Math.random().toString(36).substr(2, 9)}`,
	).current;
	console.log(
		`%c[ConnectionStatus ${instanceId}] Component instance`,
		debugColors.connectionStatus,
	);
	...
	useEffect(() => {
		console.log(
			`%c[ConnectionStatus ${instanceId}] Initial connection effect started`,
			debugColors.connectionStatus,
		);
		...
		const timer = setTimeout(() => {
			console.log(
				`%c[ConnectionStatus ${instanceId}] Setting initial connection state`,
				debugColors.connectionStatus,
			);
			setStatus("connected");
			setLatency(35);
		...
		const updateNetworkQuality = () => {
			...
			console.log(
				`%c[ConnectionStatus ${instanceId}] updateNetworkQuality called with random value: ${rand}`,
				debugColors.connectionStatus,
			);
			...
			if (rand > 0.9) {
				...
				console.log(
					`%c[ConnectionStatus ${instanceId}] Simulating connection interruption`,
					debugColors.connectionStatus,
				);
				...
				const reconnectTimer = setTimeout(() => {
					console.log(
						`%c[ConnectionStatus ${instanceId}] Simulating reconnection`,
						debugColors.connectionStatus,
					);
					setStatus("connected");
					setQuality("fair");
				...
				const improveTimer = setTimeout(() => {
					console.log(
						`%c[ConnectionStatus ${instanceId}] Simulating connection improvement`,
						debugColors.connectionStatus,
					);
					setQuality("good");
					setLatency(45);
			...
			if (rand > 0.7) {
				console.log(
					`%c[ConnectionStatus ${instanceId}] Setting quality to fair`,
					debugColors.connectionStatus,
				);
				setQuality("fair");
				setLatency(75);
				...
				console.log(
					`%c[ConnectionStatus ${instanceId}] Setting quality to good`,
					debugColors.connectionStatus,
				);
				setQuality("good");
				setLatency(40);
				...
				console.log(
					`%c[ConnectionStatus ${instanceId}] Setting quality to excellent`,
					debugColors.connectionStatus,
				);
		...
		const qualityInterval = setInterval(() => {
			console.log(
				`%c[ConnectionStatus ${instanceId}] Quality interval triggered`,
				debugColors.connectionStatus,
			);
		...
		return () => {
			console.log(
				`%c[ConnectionStatus ${instanceId}] Cleaning up timers`,
				debugColors.connectionStatus,
			);
			clearTimeout(timer);
	...
	const getStatusColor = useCallback(() => {
		console.log(
			`%c[ConnectionStatus ${instanceId}] getStatusColor called with status: ${status}, quality: ${quality}`,
			debugColors.connectionStatus,
		);
	...
	const getStatusIcon = useCallback(() => {
		console.log(
			`%c[ConnectionStatus ${instanceId}] getStatusIcon called with status: ${status}`,
			debugColors.connectionStatus,
		);
	...
	const getStatusText = useCallback(() => {
		console.log(
			`%c[ConnectionStatus ${instanceId}] getStatusText called with status: ${status}, quality: ${quality}, latency: ${latency}`,
			debugColors.connectionStatus,
		);
	...
	const getTooltipText = useCallback(() => {
		console.log(
			`%c[ConnectionStatus ${instanceId}] getTooltipText called with status: ${status}, quality: ${quality}, latency: ${latency}`,
			debugColors.connectionStatus,
		);
		...
		switch (status) {
			case "connected":
				switch (quality) {
					case "excellent":
						return `Excellent connection: ${latency}ms latency`;
					case "good":
						return `Good connection: ${latency}ms latency`;
					case "fair":
	...
	console.log(
		`%c[ConnectionStatus ${instanceId}] Before creating badge content, render #${renderCount}`,
		debugColors.connectionStatus,
	);
	...
	const badgeContent = (
		<Badge
			variant="outline"
			className={cn(
				"flex h-7 items-center gap-1.5 px-2 py-1 transition-all",
	...
	console.log(
		`%c[ConnectionStatus ${instanceId}] Before final render, showTooltip: ${showTooltip}`,
		debugColors.connectionStatus,
	);
...
const ConnectionStatusMemo = memo(
	...
	(prevProps, nextProps) => {
		...
		console.log(
			"%c[ConnectionStatus] Memo comparison:",
			debugColors.connectionStatus,
			{
				prevProps,
				nextProps,
