...
const PlayerAvatarComponent = ({
	...
	console.log(
		`%c[PlayerAvatar ${instanceId}] Component instance`,
		debugColors.playerAvatar,
	);
	...
	useDebugState(
		imageError,
		"imageError",
		`PlayerAvatar ${instanceId}`,
		debugColors.playerAvatar,
	);

	const handleImageError = useCallback(() => {
		console.log(
			`%c[PlayerAvatar ${instanceId}] Image error handler called`,
			debugColors.playerAvatar,
		);
		setImageError(true);
	}, [instanceId]);
	...
	const getSize = useCallback(() => {
		...
		console.log(
			`%c[PlayerAvatar ${instanceId}] getSize() called, returning: ${result}`,
			debugColors.playerAvatar,
		);
		return result;
	}, [size, instanceId]);
	...
	const getStatusColor = useCallback(() => {
		console.log(
			`%c[PlayerAvatar ${instanceId}] getStatusColor() called with status: ${player.status}`,
			debugColors.playerAvatar,
		);
	...
	const getStatusText = useCallback(() => {
		console.log(
			`%c[PlayerAvatar ${instanceId}] getStatusText() called with status: ${player.status}`,
			debugColors.playerAvatar,
		);
	...
	const getRoleBadge = useCallback(() => {
		console.log(
			`%c[PlayerAvatar ${instanceId}] getRoleBadge() called with role: ${player.role}, isHost: ${player.isHost}`,
			debugColors.playerAvatar,
		);
		...
		switch (player.role) {
			case "host":
				return (
					<Badge
						variant="outline"
						className="flex items-center gap-1 border-yellow-500 text-yellow-500"
					>
	...
	console.log(
		`%c[PlayerAvatar ${instanceId}] Preparing to create avatarElement`,
		debugColors.playerAvatar,
	);
	...
	const avatarElement = (
		<div className="relative">
			...
			{player.isMuted && (
				<div className="-bottom-1 -right-1 absolute rounded-full bg-red-500 p-1">
			...
			{player.rank && player.rank <= 3 && (
				<div
					className={cn(
						"-top-2 -left-2 absolute flex h-5 w-5 items-center justify-center rounded-full font-bold text-white text-xs",
						player.rank === 1
							? "bg-yellow-500"
							: player.rank === 2
								? "bg-gray-400"
	...
	console.log(
		`%c[PlayerAvatar ${instanceId}] Preparing to create tooltipContent`,
		debugColors.playerAvatar,
	);
		...
		<TooltipContent side="top" align="center">
			<div className="text-center">
				...
				{player.streak && player.streak > 1 && (
					<p className="text-orange-500 text-sm">{player.streak}x streak!</p>
				)}
				{player.status && (
	...
	console.log(
		`%c[PlayerAvatar ${instanceId}] Before final render, render #${renderCount}`,
		debugColors.playerAvatar,
	);
	...
	return (
		<div className={cn("relative flex flex-col items-center", className)}>
			...
			{showName && (
				<span className="mt-1 font-medium text-xs">{player.name}</span>
			)}
