...
export default function LoginPage() {
  ...
  const onRegisterSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      const registrationAttempts = sessionStorage.getItem('registration_attempts') || "0";
      const attempts = parseInt(registrationAttempts, 10) + 1;
      sessionStorage.setItem('registration_attempts', attempts.toString());
      
      // Apply rate limiting (for demo purposes)
      if (attempts > 5) {
        const lastAttemptTime = sessionStorage.getItem('last_registration_attempt') || "0";
        const cooldownTime = parseInt(lastAttemptTime, 10) + (30 * 60 * 1000); // 30 minutes
        
        if (Date.now() < cooldownTime) {
          const minutesRemaining = Math.ceil((cooldownTime - Date.now()) / (60 * 1000));
      ...
      sessionStorage.setItem('last_registration_attempt', Date.now().toString());
      ...
      toast({
        ...
        description: "A verification link has been sent to your email address. Please verify your account to continue.",
        variant: "success",
      });
      ...
      logger.critical("AUTH CRITICAL: Unhandled exception during registration", { 
        error: error instanceof Error ? error.message : String(error)
      });
                    ...
                    <FormField
                      ...
                      render={({ field }: { field: ControllerRenderProps<RegisterFormValues, "password"> }) => (
                        <FormItem>
                          ...
                          </FormControl>                            {/* Password strength meter */}
                            <div className="mt-2">
                              <Progress 
                              ...
                              <div className="mt-3 space-y-2 text-sm">
                                ...
                                <ul className="space-y-1 text-xs">
                                  <li className={`flex items-center gap-2 ${password.length >= 8 ? "text-green-500" : "text-red-500"}`}>
                                    {password.length >= 8 ? (
                                  ...
                                  <li className={`flex items-center gap-2 ${/[0-9]/.test(password) ? "text-green-500" : "text-red-500"}`}>
                                    {/[0-9]/.test(password) ? (
                                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                      </svg>
                                  ...
                                  <li className={`flex items-center gap-2 ${/[\W_]/.test(password) ? "text-green-500" : "text-red-500"}`}>
                                    {/[\W_]/.test(password) ? (
                                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                      </svg>
                                  ...
                                  <li className={`flex items-center gap-2 ${!/\s/.test(password) ? "text-green-500" : "text-red-500"}`}>
                                    {!/\s/.test(password) ? (
                                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                      </svg>
              ...
              if (storedLogs.length > 0) {
                return (
                  <div className="mt-4 border-t border-gray-700 pt-2">
                    <h5 className="font-bold text-xs mb-2">Persisted Critical Logs</h5>
                    <div className="space-y-1 text-red-400">
                      {storedLogs.slice(0, 5).map((log: any, i: number) => (
                        <div key={`crit-${i}`} className="whitespace-pre-wrap break-all">
