 ...
 */
export function useModels(provider: string, options: UseModelsOptions = {}): UseModelsReturn {
  const {
    autoFetch = false,
    cacheTimeToLive,
  ...
  }, [cacheTimeToLive, apiKeyVerificationTTL, setCacheTimeToLive, setApiKeyVerificationTTL]);
  
  // Check if the cache is stale (expired but still usable with background refresh)
  const isCacheStale = useCallback((providerName: string, apiKey: string): boolean => {
    ...
    if (isCacheExpired(providerName, apiKey)) {
      ...
      return cacheAge <= (cacheTimeToLive || 24 * 60 * 60 * 1000) + cacheGracePeriod;
    }
  ...
  // Calculate when the cache will expire
  const getCacheExpiryTime = useCallback((providerName: string, apiKey: string): Date | null => {
    const keyHash = apiKey.substring(0, 8);
    const cacheKey = `${providerName}:${keyHash}`;
  ...
  const getLastCacheUpdate = useCallback((providerName: string, apiKey: string): Date | null => {
    const keyHash = apiKey.substring(0, 8);
    const cacheKey = `${providerName}:${keyHash}`;
    const cache = modelsCache[cacheKey];
  ...
  const parseModelsResponse = (data: any, providerName: string): AIModel[] => {
    ...
    try {
      ...
      throw new Error(`Failed to parse models: ${e instanceof Error ? e.message : 'Unknown error'}`);
    }
    
    return parsedModels;
  };
  
  const fetchModelsFromAPI = async (providerName: string, apiKey: string): Promise<AIModel[]> => {
    ...
    if (ongoingFetchRef.current[fetchKey]) {
      console.log(`[useModels] Fetch already in progress for ${providerName}, skipping duplicate request`);
      throw new Error('A fetch operation is already in progress for this provider');
    }
  ...
  const fetchModels = useCallback(async (
    ...
    try {
      ...
      if (cachedModels && (!cacheExpired || (cacheStale && !forceRefresh)) && !forceRefresh) {
        console.log(`[useModels] Using ${cacheExpired ? "stale" : "valid"} cached models for ${providerName}`);
        ...
        if (cacheStale && timeSinceLastAttempt > 10 * 60 * 1000 && !background) {
          console.log(`[useModels] Cache is stale, triggering background refresh for ${providerName}`);
          lastAutoRefreshAttempt.current[providerName] = now;
          
          // Fire and forget - don't await this
          fetchModels(providerName, apiKey, { forceRefresh: true, background: true })
            .catch(e => console.log(`[useModels] Background refresh failed: ${e.message}`));
        }
      ...
      console.log(`[useModels] Fetching fresh models for ${providerName} (${background ? "background" : "foreground"})`);
      ...
      const errorMsg = e instanceof Error ? e.message : 'Unknown error fetching models';
      
      // Only show errors for foreground operations
      if (!background) {
        console.error(`[useModels] Error fetching models for ${providerName}:`, e);
        setError(errorMsg);
   ...
   */
  const verifyApiKey = useCallback(async (providerName: string, apiKey: string): Promise<boolean> => {
    ...
    if (isAPIKeyVerified(providerName, apiKey)) {
      console.log(`[useModels] API key already verified for ${providerName}, skipping verification`);
      return true;
    }
    
    try {
      // Check if verification is expired but we have cached models
      const keyVerificationExpired = isKeyVerificationExpired(providerName, apiKey);
      const cachedModels = getModelsFromCache(providerName, apiKey);
      ...
      if (keyVerificationExpired && cachedModels) {
        ...
        fetchModels(providerName, apiKey, { forceRefresh: true, background: true })
          .catch(e => console.log(`[useModels] Background verification refresh failed: ${e.message}`));
      ...
      const fetchedModels = await fetchModels(providerName, apiKey, { forceRefresh: true });
      return fetchedModels.length > 0;
    } catch (e) {
      console.error(`[useModels] API key verification failed for ${providerName}:`, e);
      return false;
    }
  }, [fetchModels, isAPIKeyVerified, isKeyVerificationExpired, getModelsFromCache, setVerifiedAPIKey]);
