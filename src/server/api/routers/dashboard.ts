...
import { and, asc, desc, eq, gte, lte, isNotNull, sql } from "drizzle-orm";
import { z } from "zod";
import { chatSessions, flashcardDecks, flashcards, gameAnswers, gameLobbies, gamePlayers, gameRounds, studyStats } from "~/server/db/schema";
    ...
    try {
      ...
      const totalReviewed = await ctx.db
        ...
        .where(and(
          ...
          isNotNull(flashcards.lastReview)
        ));
      ...
      const todayReviewed = await ctx.db
        ...
        .where(and(
          eq(flashcards.userId, userId),
          gte(flashcards.lastReview, today)
        ));
