...
export const flashcards = createTable(
  ...
  (t) => ({
    ...
    userId: varchar("user_id", { length: 36 })
      ...
      .references(() => users.id, { onDelete: "cascade" }),
    deckId: uuid("deck_id")
      .references(() => flashcardDecks.id, { onDelete: "cascade" }), // Nullable if card doesn't belong to a deck
...
export const gameLobbies = createTable(
  ...
  (t) => ({
    ...
    flashcardDeckId: uuid("flashcard_deck_id")
      .references(() => flashcardDecks.id, { onDelete: "set null" }),
    createdAt: timestamp("created_at", { withTimezone: true })
      .default(sql`CURRENT_TIMESTAMP`)
      .notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
...
export const feedbackRelations = relations(feedback, ({ one }) => ({
  message: one(chatMessages, { // Changed to reference merged chatMessages table
...
export const gameLobbiesRelations = relations(gameLobbies, ({ one, many }) => ({
  ...
  flashcardDeck: one(flashcardDecks, { // Relation to the deck used for the game
