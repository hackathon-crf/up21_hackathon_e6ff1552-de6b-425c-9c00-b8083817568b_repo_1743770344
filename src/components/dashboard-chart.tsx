    ...
    <ChartContainer config={chartConfig}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData}>
          ...
          <XAxis dataKey="name" />
          <YAxis />
          <Tooltip content={(props) => <CustomTooltip {...props} formatTimeValue={formatTimeValue} />} />
          <Legend />
...
function CustomTooltip({ active, payload, label, formatTimeValue }: CustomTooltipProps) {
  ...
  return (
    <div className="bg-background border border-border/50 rounded-lg shadow-xl px-2.5 py-1.5 min-w-[8rem] text-xs">
      <div className="font-medium">{label || "Unknown"}</div>
      <div className="space-y-1 mt-1.5">
        {payload.map((item, index) => (
          <div key={item.dataKey || `item-${index}`} className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full" style={{ backgroundColor: item.fill || "#888" }} />
            <div className="text-muted-foreground">{item.dataKey || "Value"}:</div>
