...
export class AIService {
  ...
  private isSupportedProvider(provider: string): boolean {
    return ["openai", "mistral", "anthropic", "gemini", "openrouter"].includes(provider);
  }
  ...
  private async *processMistralStream(stream: any): AsyncGenerator<AIStreamChunk, void, unknown> {
    try {
  ...
  private async *processOpenAIStream(stream: any): AsyncGenerator<AIStreamChunk, void, unknown> {
    try {
      for await (const chunk of stream) {
