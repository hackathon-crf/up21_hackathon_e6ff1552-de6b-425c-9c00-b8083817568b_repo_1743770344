 ...
 */
export async function fetchTrainingStreak(userId: string): Promise<SummaryCard> {
...
export async function fetchGameScore(userId: string): Promise<SummaryCard> {
  ...
  const previousAverage = scores.length > 1 ? 
    Math.round(scores.slice(1).reduce((sum, score) => sum + score, 0) / (scores.length - 1)) : 
    latestScore;
  
  const improvement = previousAverage > 0 ? 
    Math.round(((latestScore - previousAverage) / previousAverage) * 100) : 0;
 ...
 */
export async function fetchWeeklyProgress(userId: string): Promise<WeeklyProgressData[]> {
  const supabase = createBrowserClient();
  const days = 7;
  ...
  if (cardsError || gamesError || chatsError) {
    console.error("Error fetching weekly progress:", cardsError || gamesError || chatsError);
 ...
 */
export async function fetchLearningProgress(userId: string): Promise<LearningProgress[]> {
  const supabase = createBrowserClient();
  ...
  const learningProgress: LearningProgress[] = data.map((deck, index) => {
    ...
    const reviewedCards = deck.flashcards?.filter(card => card.next_review).length || 0;
 ...
 */
export async function fetchUpcomingReviews(userId: string): Promise<UpcomingReview[]> {
  const supabase = createBrowserClient();
  const now = new Date();
  ...
  const { data: remainingTodayData, error: remainingTodayError } = await supabase
    .from('flashcard')
    .select('id')
    .eq('user_id', userId)
  ...
  if (todayError || remainingTodayError || tomorrowError || weekError) {
    console.error("Error fetching upcoming reviews:", todayError || remainingTodayError || tomorrowError || weekError);
    return [];
  }
...
export async function fetchRecentActivity(userId: string): Promise<Activity[]> {
  ...
  if (gamesError || flashcardError || chatError) {
    console.error("Error fetching recent activity:", gamesError || flashcardError || chatError);
    return activities;
  }
  ...
  flashcardData?.forEach(card => {
    const deckName = card.deck_id ? (deckNames.get(card.deck_id) || "Unnamed deck") : "Unassigned cards";
    activities.push({
 ...
 */
export async function fetchTrainingRecommendations(userId: string): Promise<Recommendation[]> {
  const supabase = createBrowserClient();
  ...
  if (!flashcardStats || flashcardStats.length < 5) {
    recommendations.push({
      ...
      description: "Start by creating a deck of flashcards on a topic you want to learn.",
      icon: "plus-circle",
    });
  }
  ...
  if (flashcardStats?.length && (!gameStats || gameStats.length === 0)) {
    recommendations.push({
      ...
      description: "Put your knowledge to the test with our interactive learning games.",
      icon: "gamepad-2",
    });
  }
  ...
  recommendations.push({
    ...
    description: "Check your learning stats to see how you're improving over time.",
    icon: "line-chart",
  });
